...
public class DAL implements Serializable{
    ...
    public Connection getCon() {
        try {
            ...
            String url = "jdbc:sqlserver://localhost:1433; database=BookStore";
    ...
    public boolean insertBook(String code, String title,int price) {
        try {
            con = getCon();
    ...
    public List<Book> searchFromToPrice(int from, int to) {
        ...
        try {
            ...
            while (rs.next()) {
                ...
                Book cd = new Book(rs.getString(1), rs.getString(2),
                        rs.getInt(3));
                list.add(cd);
            }         
        } catch (Exception e) {
